...
export function shapeCall(call) {
  ...
  const toolNames = new Set(toolInvocations(call).map((t) => t.name))
  const transferAttempted = [...toolNames].some((n) => n.startsWith('transfer_to')) || truthy(extracted.transfer_attempted)
  ...
  return {
    ...
    transferAttempted,
    transferConnected: truthy(extracted.transfer_connected),
...
export function toolInvocations(call) {
  ...
  return turns
    ...
    .map((t) => ({ name: t.name, args: parseArgs(t.arguments) }))
}
